...
def applyValuesToTemplate(workflow_config_template, params):
    ...
    for key, value in params.items():
        ...
        if len(steps) < 2:
            raise Exception("key names have to start with any of these keys, and then have the path to the parameter or output: {}".format(VALID_ROOTS))
        ...
        if (lastStepSection is not None) and isinstance(lastStepSection, dict) and (lastStepSection.get('c-l-a-s-s') is not None):
...
def writeWorkflowConfigVariations(workflow_config_template, paramsArray, destdir, fnameTemplate, paramSymbolTemplate):
