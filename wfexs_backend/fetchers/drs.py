...
def query_n2t(scheme: str, the_id: str, remote_file: URIType, metadata_array: MutableSequence[URIWithMetadata] = []) -> Tuple[URIType, MutableSequence[URIWithMetadata]]:
    ...
    if not isinstance(answer, dict):
        raise FetcherException(f'Unexpected answer for n2t.net query about {remote_file} ({scheme})')
    
    ans = answer.get('scheme')
    if ans is None:
        raise FetcherException(f'No answer for n2t.net query about {remote_file} ({scheme})')
    
    a_type = ans.get('type')
    if a_type == 'synonym':
        for_ans = ans.get('for')
        if for_ans is None:
            raise FetcherException(f'Ill-formed synonym answer for n2t.net query about {remote_file} ({scheme})')
        return query_n2t(for_ans, the_id, remote_file, metadata_array)
    
    if a_type != 'scheme':
        raise FetcherException(f'Unknown "{a_type}" answer for n2t.net query about {remote_file} ({scheme})')
    
    redir_pat = ans.get('redirect')
    if redir_pat is None:
        raise FetcherException(f'Ill-formed redirect answer for n2t.net query about {remote_file} ({scheme})')
...
def downloadContentFromDRS(remote_file:URIType, cachedFilename:AbsPath, secContext:Optional[SecurityContextConfig]=None) -> ProtocolFetcherReturn:
    ...
    if parsedInputURL.scheme != DRS_SCHEME:
        raise FetcherException(f'Unexpected scheme {parsedInputURL.scheme}, expected {DRS_SCHEME}')
